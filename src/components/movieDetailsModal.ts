...
import type { TMDBMovie, DatabaseMovie, TMDBMovieDetails } from '../types/movie';
...
export function createMovieDetailsModal(
  ...
  const posterPath = movie.poster_path ? TMDB_IMAGE_BASE_URL + movie.poster_path : '/placeholder.jpg';
  ...
  const personalRating = isDatabaseMov ? (movie as DatabaseMovie).personal_rating : null;
  const isWatched = isDatabaseMov ? (movie as DatabaseMovie).is_watched === 1 : false;
  const isFavorite = isDatabaseMov ? (movie as DatabaseMovie).is_favorite === 1 : false;
  const inWatchlist = isDatabaseMov ? (movie as DatabaseMovie).in_watchlist === 1 : false;
